...
class CMB_Meta_Box_FTG {
	...
	function __construct( $meta_box ) {
		...
		add_action( 'cmbftg_init_fields', array( &$this, 'init_fields' ) );
		...
		add_action( 'cmbftg_save_fields', array( &$this, 'save' ) );

		// add_action( 'admin_enqueue_scripts', array( &$this, 'enqueue_scripts' ) );
		// add_action( 'admin_enqueue_scripts', array( &$this, 'enqueue_styles' ) );

		add_filter( 'cmbftg_show_on', array( &$this, 'add_for_id' ), 10, 2 );
		add_filter( 'cmbftg_show_on', array( &$this, 'add_for_page_template' ), 10, 2 );
	...
	public function init_fields( $post_id = 0 ) {
		...
		foreach ( $this->_meta_box['fields'] as $key => $field ) {
			...
			$class = _cmbftg_field_class_for_type( $field['type'] );
	...
	function add() {
		...
		foreach ( (array) $this->_meta_box['pages'] as $page ) {
			$show = apply_filters( 'cmbftg_show_on', true, $this->_meta_box );
			if ( $show ) {
	...
	/**
	 * Show On Filters
	 * Use the 'cmbftg_show_on' filter to further refine the conditions under which a metabox is displayed.
		...
		<div class="cmbftg_metabox">
			...
			foreach ( $fields as $field ) :
				...
				<div class="cmb-cell-<?php echo intval( $field->args['cols'] ); ?>">
						...
						<input type="hidden" name="_cmbftg_present_<?php esc_attr_e( $field->id ); ?>" value="1" />
	...
	function save( $post_id = 0 )  {
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			if ( ! isset( $_POST['_cmbftg_present_' . $field['id'] ] ) )
				continue;
			...
			if ( ! $class = _cmbftg_field_class_for_type( $field['type'] ) ) {
				do_action('cmbftg_save_' . $field['type'], $field, $value);
			}
