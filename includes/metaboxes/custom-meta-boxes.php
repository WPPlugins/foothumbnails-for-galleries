...
function cmbftg_init() {
	...
	$meta_boxes = apply_filters( 'cmbftg_meta_boxes', array() );

	if ( ! empty( $meta_boxes ) )
		foreach ( $meta_boxes as $meta_box )
			new CMB_Meta_Box_FTG( $meta_box );

}
add_action( 'init', 'cmbftg_init' );
 ...
 * @return array
 */
function _cmbftg_available_fields() {

	return apply_filters( 'cmbftg_field_types', array(
		'text'				=> 'CMB_Text_Field',
 ...
 */
function _cmbftg_field_class_for_type( $type ) {

	$map = _cmbftg_available_fields();
 ...
 * @return string $query
 */
function cmbftg_fix_meta_query_order($query) {
...
if ( version_compare( get_bloginfo( 'version' ), '3.8', '<' ) )
	add_filter( 'query', 'cmbftg_fix_meta_query_order', 1 );