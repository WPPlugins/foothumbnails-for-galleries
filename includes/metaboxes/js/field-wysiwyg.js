...
CMB.addCallbackForClonedField( 'CMB_wysiwyg', function( newT ) {
	...
	newT.find( '.cmb-wysiwyg' ).each( function (i) {
		...
		// Placeholder markup for the new wysiwyg is stored as a prop on var cmbftg_wysiwyg_editors
		// Copy, update ids & names & insert.
		el.html( cmbftg_wysiwyg_editors[fieldId].replace( nameRegex, name ).replace( idRegex, id ) );
