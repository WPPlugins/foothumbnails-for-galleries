...
Plugin URI: http://foothumbnails.mattcromwell.com/
...
//settings to enable shortcodes in the following places:
	...
	if (isset($ftg_settings['ftg_enable_imgtitle'])) {
		function enable_image_titles($atts,$img) {
				$atts['title'] = trim(strip_tags( $img->post_title ));
				return $atts;
			}	
		add_filter('wp_get_attachment_image_attributes','enable_image_titles',10,2);
	}
...
class foothumbnail_gallery extends Foo_Plugin_Base_v2_1 {	
	...
	function create_settings_ftg() {
		...
		$settings[2] = array(
			'id' => 'ftg_enable_imgtitle',
			'title' => __('Enable the title tag in your images', 'foothumbgallery'),
			'tab' => 'intro',
			'desc' => __('By default, WordPress does not output the title tag in images. Some themes build this in, but if yours doesn\'t and you want to utilize them for your FooBox caption, enable this option.', 'foothumbgallery'),
			'type' => 'checkbox',
			'default' => 'off'
		);
